...
The first run may request administrator authorization to trust the local certificate authority and install the service that binds port 443. On macOS, service installation and idle updates use a native administrator dialog. Idle service updates can authorize while detached, but trust changes and installation triggered from Vite or the CLI require an attached terminal; without one, the plugin refuses and reports the command to run. If several Vite processes start together, they wait for the same authorization flow instead of opening competing prompts. An outdated compatible service keeps serving active routes without interruption and updates automatically on the next Vite start after it becomes idle. The updater never downgrades a newer compatible installed service.
...
- Run Vite or lifecycle commands in an interactive terminal when administrator authorization is required. Without one the plugin refuses and names the command to run; on Linux the prompt appears in that terminal, and on macOS authorization uses a native administrator dialog. Idle service updates can still authorize while detached.
